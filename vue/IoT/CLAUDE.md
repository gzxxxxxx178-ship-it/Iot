...
| 状态管理 | Pinia (Setup Store 语法) — authStore + deviceStore |
...
## 六、stores/ — Pinia 状态管理 (2 个)

本项目使用 Pinia Setup Store 语法（Composition API 风格）管理全局状态。

| 文件 | 状态 | 说明 |
|------|------|------|
| `auth.js` | token, username, isLoggedIn | 认证状态。login/register/logout/setAuth/restore actions。localStorage 双写（Pinia + localStorage），保证 axios 拦截器等非组件上下文也能读取 token |
| `device.js` | devices, dataPoints, latest, stats | 设备与传感器数据。fetchDevices/fetchHistory/addDataPoint actions。Dashboard、Monitor、Screen 共享同一份数据，避免重复请求。timeLabels/tempSeries/humSeries getter 供图表组件直接使用 |
...
authStore.logout()
// 响应式读取
console.log(authStore.username, authStore.isLoggedIn)
```

### 设计原则

- **Setup Store 语法** — 与 Vue 3 Composition API 一致，ref/reactive/computed 直接映射
- **localStorage 双写** — authStore 写入时同步更新 localStorage，保证 axios 拦截器（非组件上下文）能通过 `getToken()` 读取 token
- **按需加载** — 页面进入时 `onMounted` 中调用 fetch，避免首屏不必要请求
- **utils/auth.js 保留** — 基础读写函数供 axios 拦截器使用，不依赖 Pinia 上下文
