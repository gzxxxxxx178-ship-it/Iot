	...
	async function loadDashboard() {
		...
		try {
			...
			pieData.value = distribution.map((item) => ({
				...item,
				itemStyle: {
					color: item.name === '在线' ? '#10b981' : '#ef4444'
				},
			}))
		} catch {
...
<template>
	<div class="dashboard">
		...
		<div class="dashboard-grid">
			<div class="chart-col">
				<TempHumChart :timeLabels="deviceStore.timeLabels" :tempSeries="deviceStore.tempSeries"
					:humSeries="deviceStore.humSeries" height="300px" />
			</div>
			<div class="stats-col">
				...
				<el-card class="stat-card">
					...
					<el-tag
						:type="stats.latestLinkage ? 'success' : 'info'">{{ stats.latestLinkage ? 'ON' : 'OFF' }}</el-tag>
				</el-card>
				<el-card class="stat-card">
